...
public class grunt implements GameObject
{
    ...
    public grunt(int color, int left, int top, int right, int bottom)
    {
        //rect = left top right bottom
        this.grunt = new Rect(left,top,right,bottom);
        this.damage = 15;
        this.health = 250;
        this.speed = 7;
        this.color = color;
    ...
    {
        {
            ...
            {
                this.alive = false;
                System.out.println("Attempting removal of grunt");
                sm.grunts.remove(this);
                sm.enemyCounter--;
                draw(canvas);
            }
        }
    ...
    {
        grunt.left += speed - (speed/2);
        grunt.right += speed- (speed/2);
        //zombie.top += speed-(speed/2);
        //zombie.bottom +=speed -(speed/2);
        currentPosy = grunt.bottom;
        currentPos = grunt.right -(grunt.right-grunt.left);
        // left = zombie.left;
        // right = zombie.right;
